...
<template>
  <AppDialog :model-value="modelValue" :title="t('customers.dialogs.blacklistTitle')" max-width="480" @update:model-value="emit('update:modelValue', $event)">
    <AppForm :id="formId" :schema="blacklistSchema" :initial-values="{ reason: '' }" @submit="onSubmit">
    ...
    <template #actions>
      ...
      <v-btn type="submit" :form="formId" color="error" variant="flat" :loading="loading">{{ t('customers.actions.blacklist') }}</v-btn>
    </template>
  </AppDialog>
</template>
