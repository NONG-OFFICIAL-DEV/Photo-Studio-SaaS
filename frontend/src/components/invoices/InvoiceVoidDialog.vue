...
import { voidInvoiceApi } from '@/apis/invoice.api'
...
async function onSubmit(values) {
  loading.value = true
  try {
    await voidInvoiceApi(props.invoiceId, values.reason)
    appStore.notify(t('invoices.messages.voidedSuccess'))
    emit('saved')
...
<template>
  <AppDialog :model-value="modelValue" :title="t('invoices.voidInvoice')" max-width="480" @update:model-value="emit('update:modelValue', $event)">
    <AppForm :id="formId" :schema="voidInvoiceSchema" :initial-values="{ reason: '' }" @submit="onSubmit">
    ...
    <template #actions>
      ...
      <v-btn type="submit" :form="formId" color="error" variant="flat" :loading="loading">{{ t('invoices.voidInvoice') }}</v-btn>
    </template>
  </AppDialog>
</template>
