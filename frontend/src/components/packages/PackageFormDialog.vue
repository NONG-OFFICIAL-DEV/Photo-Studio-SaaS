...
const componentTotal = computed(() =>
  components.value.filter((c) => !c.is_optional).reduce((sum, c) => sum + lineTotal(c), 0),
)
...
<template>
  ...
  >
    ...
    <AppForm :id="formId" :schema="packageSchema" :initial-values="initialValues" @submit="onSubmit">
      <template #default="{ errors, values, setFieldValue }">
        ...
        <v-row>
          ...
          <v-col cols="12" sm="4">
            <v-text-field
              :model-value="values.override_price"
              :label="t('packages.overridePrice')"
              type="number"
              step="0.01"
              prefix="$"
              :error-messages="errors.override_price"
              @update:model-value="setFieldValue('override_price', $event)"
            />
          </v-col>
        </v-row>
        ...
        <div class="d-flex justify-end mb-4">
          <div style="min-width: 220px">
            <div class="text-body-2 d-flex justify-space-between">
              <span>{{ t('packages.componentTotal') }}</span
              ><span>{{ formatCurrency(componentTotal) }}</span>
            </div>
            <div class="text-h6 d-flex justify-space-between">
              <span>{{ t('packages.finalPrice') }}</span
              ><span>{{ formatCurrency(finalPrice(values)) }}</span>
            </div>
          </div>
        </div>
      </template>
